        ...
        {activeTab === 'sg' && (
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 16px' }}>
              ...
              <Stepper label="셀 접기 기준"        value={settings.cellFold === 0 ? '∞' : settings.cellFold}       sub="0=항상 펼침" onMinus={() => { S.settings.cellFold = Math.max(0,Math.min(20,settings.cellFold-1)); S.expandedCells=new Set(); save(); syncSettings(); window.renderAll?.(); }} onPlus={() => { S.settings.cellFold = Math.max(0,Math.min(20,settings.cellFold+1)); S.expandedCells=new Set(); save(); syncSettings(); window.renderAll?.(); }} />
