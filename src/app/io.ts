...
const notify = (msg: string, type: string | boolean = 'success') => getStore().notify?.(msg, type);

function tsvEncode(val: unknown): string {
  const s = String(val || '');
...
function parseTSVLine(line: string): string[] {
  const fields: string[] = []; let cur = '', inQ = false, i = 0;
  while (i < line.length) {
    const ch = line[i];
    if (inQ) {
...
export function parseTSVFull(raw: string): string[][] {
  const lines = raw.replace(/\r\n/g, '\n').replace(/\r/g, '\n');
  const rows: string[][] = []; let cur = '', inQ = false;
...
function buildTSV(): string {
  ...
  getStore().items.forEach(it => { rows.push(FIELDS.map(f => tsvEncode((it as Record<string, unknown>)[f] || '')).join('\t')); });
  return rows.join('\n');
}

export function expClip(): void {
  const tsv = buildTSV();
...
export function expXLS(): void {
  ...
  getStore().items.forEach(it => {
    h += '<tr>';
    FIELDS.forEach(f => {
      const v  = (((it as Record<string, unknown>)[f] as string) || '').replace(/\n/g, ' ');
...
export function expHTML({ fluid = false } = {}): void {
  const isFluid = !!fluid;
  ...
  const c       = getColors();
  const cMap    = c as unknown as Record<string, string | number>;

  const gm: Record<string, Record<string, boolean>> = {};
  const cm2: Record<string, Record<string, boolean>> = {};
  items.forEach(it => {
  ...
  const sk = (a: string, b: string): number => { try { return a.localeCompare(b, 'ko'); } catch { return a < b ? -1 : 1; } };
  const uniqSorted = (field: keyof Item, arr: Item[]): string[] => {
    const seen = new Set<string>(), r: string[] = [];
    arr.forEach(it => { const v = String(it[field] || '(미분류)'); if (!seen.has(v)) { seen.add(v); r.push(v); } });
    return r.sort(sk);
  };
  const groups = uniqSorted('group', store.items).filter(g => !!gm[g]);
  const cats   = uniqSorted('category', store.items).filter(cat => !!cm2[cat]);
  const gsubs: Record<string, string[]> = {}, csubs: Record<string, string[]> = {};
  ...
  const prioCount: Record<string, number> = { high: 0, mid: 0, low: 0 };
    ...
    csubs[cn].forEach((scn, sci) => {
      ...
      groups.forEach(gn => { gsubs[gn].forEach(sg => {
        ...
        ci.forEach(itm => {
          ...
          const pHex = (cMap[`p${pkC}`] as string) || '#888';
          const pBg  = (cMap[`p${pkC}Bg`] as string) || '#eee';
          const cs2 = `${getPresetCSS(ss.priorityStyles[pk as keyof typeof ss.priorityStyles], pHex, pBg)};border-radius:${ss.cardRadius}px;margin-bottom:${ss.cardGap}px;padding:5px 7px`;
...
export function expMdZip(): void {
  ...
  const files = mdItems.map(it => ({
    ...
    content: it.mdContent as string
  }));
...
export function impMdFiles(e: Event): void {
...
function buildZip(files: ZipFile[]): Uint8Array {
  ...
  const localHeaders: Uint8Array[] = [], centralDirs: Uint8Array[] = [];
  let offset = 0;
  ...
  const eocd = new Uint8Array([0x50, 0x4B, 0x05, 0x06, 0x00, 0x00, 0x00, 0x00]
    .concat(u16le(files.length)).concat(u16le(files.length))
    .concat(u32le(cdSize)).concat(u32le(cdStart)).concat([0x00, 0x00]));
